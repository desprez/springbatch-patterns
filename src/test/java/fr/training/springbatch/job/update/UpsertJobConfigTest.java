...
class UpsertJobConfigTest {
    ...
    void launch_UpsertJob_nominal_should_success() throws Exception {
        ...
        final Optional<StepExecution> executionOpt = execution.getStepExecutions().stream().filter(e -> "upsert-step".equals(e.getStepName())).findFirst();
        assertThat(executionOpt).isPresent();
        final StepExecution stepExec = executionOpt.get();

        assertThat((int) stepExec.getReadCount()).isEqualTo(37);
        assertThat((int) stepExec.getWriteCount()).isEqualTo(37);
    }
}
