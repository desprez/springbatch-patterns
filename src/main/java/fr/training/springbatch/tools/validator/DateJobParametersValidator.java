...
public class DateJobParametersValidator implements JobParametersValidator, InitializingBean {
    ...
    public DateJobParametersValidator(final String parameterKey, final boolean pastDateAllowed, final boolean futureDateAllowed) {
    ...
    public void validate(final JobParameters parameters) throws JobParametersInvalidException {
        ...
        if (!pastDateAllowed) {
            if (paramValue.before(today)) {
                throw new JobParametersInvalidException(String.format("Past date value not allowed for %s parameter", parameterKey));
            }
        }
        if (!futureDateAllowed) {
            if (paramValue.after(today)) {
                throw new JobParametersInvalidException(String.format("Future date value not allowed for %s parameter", parameterKey));
            }
        }
    }
