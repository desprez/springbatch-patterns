 ...
 * @param <K>
 *            The class of the key value of the items being processed. Used for positioning the reader to a particular place in the input
 *
...
public abstract class ItemAccumulator<T, K extends Comparable<K>> implements ItemStream, InitializingBean {
    ...
    /**
     * Read forward through the Reader looking for the first record with a key that is equal to or greater than the key passed in.
     *
     * @param key
     *            Key to search for.
    ...
    public List<T> readNextItems(final K key) throws Exception {
        ...
        if (lastItem != null && lastItemList != null && !lastItemList.isEmpty() && checkEqualKey(lastItemList.get(0), key)) {
            return lastItemList;
        }
